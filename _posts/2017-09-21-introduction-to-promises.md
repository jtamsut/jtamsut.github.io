---
layout: post
title: "Introduction to Promises in JS"
categories: javascript promises asynchronous
---

This article explores the fundamentals of asynchronous code in JS. It then explains how to use a tool called Promises to "deal" with asynchrony in your code.
...
The single-threaded nature of JavaScript is accomplished via the **event loop**. The [event loop](https://developer.mozilla.org/en-US/docs/Web/JavaScript/EventLoop) places code that is not yet executed in a queue. The order in which code is placed on the queue is determined by the order in which the JS engine "sees" the code. When code is dequeued it is executed by the JS engine.
...
<center>**The fundamental principle behind writing asynchronous code is that we cannot assume when this code will be run**.</center>
...
This is how we pass data from Promise to `resolve` function:

![resolve-diagram]({{ "/images/resolve-value.svg" }})
...
`Promise.all` is how we can execute more than one Promise at once. The `Promise.all()` method returns a single Promise that resolves only when all of the Promises passed to it have resolved or when no promises are passed to it. It rejects with the reason of the first promise that rejects.

`Promise.all` is a way of saying "Do these few things first then do this".
